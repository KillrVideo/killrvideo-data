#!/usr/bin/env python3
"""
Generate 768-dimension test vector data using all-mpnet-base-v2 model.

This script creates test data in multiple formats (CSV, CQL, JSON) for testing
Cassandra vector search capabilities with 768-dimensional embeddings.
...
# Fixed UUIDs for reproducibility (using 768xxxxx pattern)
FIXED_UUIDS = [
    "76800000-0000-0000-0000-000000000001",
    "76800000-0000-0000-0000-000000000002",
    "76800000-0000-0000-0000-000000000003",
    "76800000-0000-0000-0000-000000000004",
    "76800000-0000-0000-0000-000000000005"
]

# Output directory
OUTPUT_DIR = Path(__file__).parent
CSV_FILE = OUTPUT_DIR / "test-data-768d.csv"
CQL_FILE = OUTPUT_DIR / "test-data-768d.cql"
JSON_FILE = OUTPUT_DIR / "test-data-768d.json"
...
def generate_embeddings(model: SentenceTransformer, phrases: List[str]) -> List[List[float]]:
    ...
    if embeddings_list:
        ...
        if dimension != 768:
            print(f"WARNING: Expected 768 dimensions but got {dimension}", file=sys.stderr)
...
def write_csv(data: List[Tuple[str, str, List[float]]]) -> None:
    ...
    with open(CSV_FILE, 'w', newline='', encoding='utf-8') as f:
        ...
        writer.writerow(['76800000-0000-0000-0000-000000000999', 'NULL vector test', ''])
...
def write_cql(data: List[Tuple[str, str, List[float]]]) -> None:
    ...
    with open(CQL_FILE, 'w', encoding='utf-8') as f:
        ...
        f.write("-- CQL INSERT statements for 768-dimension vector test data\n")
        f.write("-- Generated using sentence-transformers/all-mpnet-base-v2 model\n")
        f.write("-- \n")
        f.write("-- Usage: cqlsh -f test-data-768d.cql\n")
        f.write("-- \n")
        f.write("-- Schema:\n")
        f.write("-- CREATE TABLE IF NOT EXISTS default_keyspace.vectors_768 (\n")
        f.write("--     id uuid PRIMARY KEY,\n")
        f.write("--     name text,\n")
        f.write("--     embedding vector<float, 768>\n")
        f.write("-- );\n\n")
        ...
        for uuid, name, embedding in data:
            ...
            f.write(f"INSERT INTO default_keyspace.vectors_768 (id, name, embedding)\n")
            f.write(f"VALUES ({uuid}, '{name}', {vector_str});\n\n")

        # Add NULL vector test case
        f.write("-- NULL vector test case\n")
        f.write("INSERT INTO default_keyspace.vectors_768 (id, name, embedding)\n")
        f.write("VALUES (76800000-0000-0000-0000-000000000999, 'NULL vector test', NULL);\n")
...
def write_json(data: List[Tuple[str, str, List[float]]]) -> None:
    ...
    with open(JSON_FILE, 'w', encoding='utf-8') as f:
        ...
        # Add NULL vector test case
        null_obj = {
            'id': '76800000-0000-0000-0000-000000000999',
...
def main():
    ...
    try:
        print("=" * 70)
        print("768-Dimension Vector Test Data Generator")
